...
namespace Bonsai
{
    ...
    public static class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            var bitcoinChains = 
                from bsv in BlockGenerator<Block<BSV<string, Transaction<string, string>>>>.Generate(BlockHelper.ParseMagicNumber(Bitcoin.BSV), Configuration.DataDirPath)
                from bch in BlockGenerator<Block<BCH<string, Transaction<string, string>>>>.Generate(BlockHelper.ParseMagicNumber(Bitcoin.BCH), Configuration.DataDirPath)
                from btc in BlockGenerator<Block<BTC<string, Transaction<string, string>>>>.Generate(BlockHelper.ParseMagicNumber(Bitcoin.BTC), Configuration.DataDirPath)
                select new { bsv, bch, btc };  // Could be any combination of chains

            bitcoinChains.Do(block => {
                    Console.WriteLine($"Block: {((Block<BSV<string, Transaction<string, string>>>)block.bsv).Height}, Hash: {((Block<BSV>)block.bsv).Hash}, Previous Hash: {((Block<BSV>)block.bsv).PreviousBlockHash}");
